...
describe("BinarySearchTree", function () {
    ...
    describe("insert(element) method", function () {
        it("inserts an elment to the  the bst", function () {
            assert.deepEqual(bst.insertAll(10));
            assert.deepEqual(bst.insertAll(5));
            assert.deepEqual(bst.insertAll(3));
            assert.deepEqual(bst.insertAll(7));
            assert.deepEqual(bst.insertAll(1));
            assert.deepEqual(bst.insertAll(4));
            assert.deepEqual(bst.insertAll(6));
            assert.deepEqual(bst.insertAll(8));
        });
    });

    describe("read in to array ", function () {
        it("read In to Array ", function () {
            assert.deepEqual(bst.readIntoArray(10));
            assert.deepEqual(bst.readIntoArray(5));
            assert.deepEqual(bst.readIntoArray(3));
            assert.deepEqual(bst.readIntoArray(7));
            assert.deepEqual(bst.readIntoArray(1));
            assert.deepEqual(bst.readIntoArray(4));
            assert.deepEqual(bst.readIntoArray(6));
            assert.deepEqual(bst.readIntoArray(8));
        });
    });
    describe("printnamecomparator", function() {
        it("returns a string that describes how comparison between names alphabetically", function() {
            let emp = [
                new Employee("George", 45000, 1996, 11, 5),
                new Employee("Dave", 50000, 2000, 1, 3),
                new Employee("Richard", 40000, 2001, 2, 7)
                ];
            assert.equal(emp.comparator(), "Dave" > "George"> "Richarge");
